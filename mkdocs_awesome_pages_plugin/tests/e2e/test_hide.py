...
class TestHide(E2ETestCase):
    def test(self):
        navigation = self.mkdocs(
            self.config,
            [("a", ["1.md", self.pagesFile(hide=True)]), ("b", ["2.md"])],
        )
    ...
    def test_single_root_section(self):
        navigation = self.mkdocs(
            self.config, [("section", ["page.md", self.pagesFile(hide=True)])]
        )
    ...
    def test_propagate(self):
        navigation = self.mkdocs(
            self.config,
            [("a", [("b", ["page.md", self.pagesFile(hide=True)])])],
        )
    ...
    def test_in_root(self):
        with self.assertWarns(HideInRootHasNoEffect):
            navigation = self.mkdocs(
                self.config, ["page.md", self.pagesFile(hide=True)]
            )
